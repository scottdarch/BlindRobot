...
// Allowed deviation (in degrees) relative to target angle that must be achieved before driving
// straight
...
void setup()
{
    ...
    compass.writeReg(
        LSM303::CRB_REG_M,
        CRB_REG_M_2_5GAUSS); // +/- 2.5 gauss sensitivity to hopefully avoid overflow problems
...
void loop()
{
    ...
    // If the Zumo has turned to the direction it wants to be pointing, go straight and then do
    // another turn
...
{
    float x_scaled =
        2.0 * (float)(v.x - compass.m_min.x) / (compass.m_max.x - compass.m_min.x) - 1.0;
    float y_scaled =
        2.0 * (float)(v.y - compass.m_min.y) / (compass.m_max.y - compass.m_min.y) - 1.0;
